...
public class WheelPicker extends View implements IDebug, IWheelPicker, Runnable {
    ...
    public WheelPicker(Context context, AttributeSet attrs, String selectedMonth, String selectedYear) {
        this(context, null);
    }
    ...
    public WheelPicker(Context context, AttributeSet attrs) {
        ...
        mPaint.setTypeface(RootValues.getMontserratRegularFont());

        if (fontPath != null) {
            Typeface typeface = RootValues.getMontserratRegularFont();
...
//        }
    ...
    private void initAppFonts() {
        RootValues.setMontserratBoldFont(
                Typeface.createFromAsset(
                        getContext().getAssets(),
                        "fonts/meftii/montserrat_bold.ttf"
                )
        );
        RootValues.setMontserratRegularFont(
                Typeface.createFromAsset(
                        getContext().getAssets(),
                        "fonts/meftii/montserrat_regular.ttf"
                )
        );
        RootValues.setMontserratSemiBoldFont(
                Typeface.createFromAsset(
                        getContext().getAssets(),
                        "fonts/meftii/montserrat_semibold.ttf"
                )
        );
    }//initAppFonts ends
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        setMeasuredDimension(resultWidth, resultHeight + 100);
    }
...
//                    mTracker.computeCurrentVelocity(1000);
    ...
    public void setData(List data) {
        if (null == data)
            throw new NullPointerException("WheelPicker's data cannot be null!");
        mData = data;
